...
if(isValidEmail && isValidNachname && isValidVorname && isValidAdresse && isValidPlz && isValidOrt && isValidTelefon && isValidEmail && isValidCheckbox){
  mail_url = 'mailto:office@greenbike.at?Subject=' + 'Anfrage' + '&Body=' + $('#exampleFormControlTextarea1').val() + '%0D%0A' + '%0D%0A' +$('#inputVorname').val() + ' ' + $('#inputNachname').val() + '%0D%0A' + $('#inputAdresse').val() + ', ' + $('#inputPlz').val() + ' ' + $('#inputOrt').val() + '%0D%0A' + $('#inputTelefon').val() + '%0D%0A' + $('#email').val()
